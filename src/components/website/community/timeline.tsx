...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div className="w-full bg-background pt-20 font-sans md:px-10" ref={containerRef}>
      <div className="mx-auto max-w-7xl px-4 py-5 md:px-8 lg:px-10">
        ...
        <p className="max-w-[600px] text-base text-muted-foreground sm:text-lg md:text-xl">
          The following is a list of features that are planned for the future of Ducket.
        </p>
      </div>
      ...
      <div ref={ref} className="relative mx-auto max-w-7xl pb-20">
          ...
          <div key={index} className="flex justify-start pt-10 md:gap-10 md:pt-40">
            <div className="sticky top-40 z-40 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm">
              <div className="absolute left-3 flex h-10 w-10 items-center justify-center rounded-full border border-border bg-background md:left-3">
                <div className="h-4 w-4 rounded-full border border-border bg-muted-foreground/40" />
              </div>
            ...
            <div className="relative w-full pl-20 pr-4 md:pl-4">
              <h3 className="mb-4 block text-left text-2xl font-bold md:hidden">{item.title}</h3>
              {item.content}{' '}
            </div>
          </div>
        ))}
        <div
          ...
          <motion.div
            ...
            className="absolute inset-x-0 top-0 w-[2px] rounded-full bg-gradient-to-t from-accent from-[0%] via-tertiary via-[10%] to-transparent"
          />
        </div>
      </div>
    </div>
